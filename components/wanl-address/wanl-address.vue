		...
		methods:{
			...
			address_item_click(swiper_index,index){
				...
				if (swiper_index === 0){//第一级
					...
					this.dataList.splice(swiper_index + 1,0,'選択')
					...
					if (area !== undefined){
						...
						let result = {}
						result['district'] = this.dataList[2]
						result['city'] = this.dataList[1]
						result['formatted_address'] = this.dataList.join('')
						...
						/*uni.request({
						    ...
						    success: (res) => {
								res.data.geocodes[0]['district'] = this.dataList[2]
								res.data.geocodes[0]['city'] = this.dataList[1]
								res.data.geocodes[0]['formatted_address'] = this.dataList.join('')
