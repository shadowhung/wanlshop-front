<template>
	<view>
		<view class="wanl-share">
			...
			<view class="footer" @tap="close"><text>キャンセル</text></view>
		</view>
	</view>
</template>
...
<script>
	/**
	 * WanlShare 分享 
	 * @description 分享組件 SHOP https://www.shop.com（除万联官方内嵌系統，未经授權严禁使用）
	 * @著作權：WanlShop 登記號：2020SR0255711 版本：V1.0.0
	 * @property {Number} scrollAnimation 滚動位置
	 * @property {Number} shareType 分享類型 0.圖文 1.纯文字 2.纯圖片 5.小程序
	 * @property {String} shareTitle 分享标题
	 * @property {String} shareText 分享詳情
	 * @property {String} image 分享圖片 如果是圖文分享，且是ios平台，20kb
	 * @property {String} href 分享鏈接
	 * @event {Function} change 關閉弹窗
		...
		methods: {
			async share(e) {
				if (e.id == 'poster') {
					...
					// #ifdef H5
					this.$wanlshop.msg('暂不支持，請手動複製');
					// #endif
					// #ifndef H5
					uni.setClipboardData({
						data: this.href,
						success: () => {
							this.$wanlshop.msg('複製成功');
						}
					});
					// #endif
				} else if (e.id == 'report') {
					...
					// if(shareOPtions.type === 0 && plus.os.name === 'iOS'){//如果是圖文分享，且是ios平台，则压缩圖片 
