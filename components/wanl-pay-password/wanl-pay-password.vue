<template>
	<view class="best-modal digital-keyboard-modal" :class="{'best-modal-active':_show}">
		...
		<view class="best-modal-content">
			<view class="dk-title">輸入密碼</view>
			<view class="dk-subtitle">請輸入支付密碼</view>
			...
			<view class="pwd-forget">
				<text v-if="_forget" @tap="forgetPwd">没有密碼？</text> <text v-if="_forget" @tap="forgetPwd">パスワードをお忘れですか</text>
			</view>
	...
	export default {
		...
		methods: {
			...
			getKeyNumber: function(val) {
				if (val === '' || (val != -1 && this.activeInput == this.digits)) { //空或者已经達到最大值
					return false;
				} else if (val != -1) { //數字輸入
					this.activeInput++
					this.paymentPwd += val;
					if (this.activeInput == this.digits) { //驗證密碼正確性
