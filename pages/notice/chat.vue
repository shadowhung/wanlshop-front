			 ...
			 :scroll-into-view="scrollToView" @scrolltoupper="loadHistory" upper-threshold="50">
				...
				<view v-for="(row, index) in msgList" :key="index" :id="'msg' + row.id">
					...
					<view v-if="row.type == 'sys'">
						<!-- 系統消息 -->
						<view class="cu-info round">相手がメッセージを撤回した!</view>
					</view>
				</view>
			</scroll-view>
		</view>
		...
		<view class="chatTips bg-white radius-bock margin-lr-bj padding-sm" v-if="goods && isGoods">
			...
			<view class="details">
				...
				<view class="flex justify-between">
					...
					<view>
						<button class="cu-btn round bg-orange sm" @tap="sendGoods(goods)">商人を送る</button>
					</view>
				</view>
			</view>
		</view>
		...
		<view class="WANL-MODAL">
				...
				<view class="cu-dialog bg-white" @tap.stop="">
					<view class="wanl-modal">
						<view class="head padding-bj">
							<view class="content"><view class="text-lg">閲覧した製品</view></view>
							...
							<view class="item goods" v-for="(item, index) in goodsData" :key="index">
								...
								<view class="details">
									...
									<view class="flex justify-between info">
										...
										<button class="cu-btn round line-orange sm" @tap="sendGoods(item.goods)">商品を送る</button>
									</view>
								</view>
							</view>
						</scroll-view>
			...
			<view class="cu-modal bottom-modal" :class="modalName == 'order' ? 'show' : ''" @tap="hideModal">
				<view class="cu-dialog bg-bgcolor" @tap.stop="">
					<view class="wanl-modal">
						<view class="head padding-bj">
							<view class="content"><view class="text-lg">購入した注文書</view></view>
							<view class="close wlIcon-31guanbi" @tap="hideModal"></view>
						</view>
						<scroll-view class="scroll-y" scroll-y="true">
							<view class="item bg-white radius-bock margin-tb-sm padding-bj" v-for="(item, index) in orderData" :key="item.id" @tap="sendOrder(item)">
								...
								<view class="flex justify-end">
									<button class="cu-btn round sm line-orange">注文書を送ります</button>
								</view>
							</view>
						</scroll-view>
						<view class="foot padding-lr-bj"><button class="cu-btn bg-gradual-orange round text-bold complete" @tap="hideModal">実施する</button></view>
...
export default {
	data() {
		return {
			...
			hideMore: true,
			modalName: null,
			stateText: ["お支払いを待っています", "支払い完了", "到着予定", "コメント", "払い戻し注文", "注文完了", "取引終了"],
			refundStatusText: ["返金なし", "払い戻し", "返品待ち", "払い戻しが完了しました", "払い戻しは終了しました", "払い戻しは拒否されました"],
	...
	onLoad(option) {
		...
		this.$api.post({
			...
			success: res => {
				...
				this.$wanlshop.title(res.shopname + (res.isOnline == 1 ? '-オンライン':'-オフライン'));
				// 初始界面
	...
	methods: {
		...
		loadHistory(e) {
			...
			this.isHistoryLoading = true; //參數作為进入請求标识，防止重複請求
			this.scrollAnimation = false; //關閉滑動動画
			let view_id = this.msgList[0].id; //記住第一個信息ID
			//請求曆史記錄效果
			this.$api.post({
				...
				success: res => {
					...
					//这段代碼很重要，不然每次加載曆史數據都會跳到顶部
					this.$nextTick(() => {
						this.scrollToView = 'msg' + view_id;
						this.$nextTick(() => {
							this.scrollAnimation = true; //恢複滚動動画
						});
					});
					// 判断尾頁
		...
		//处理圖片尺寸，如果不处理宽高，新进入頁面加載圖片时候會闪
		setPicSize(content) {
			// 让圖片最长边等於設置的最大长度，短边等比例缩小，圖片控件真实改变，区别於aspectFit方式。
			let maxW = uni.upx2px(350); //350是定义消息圖片最大宽度
			let maxH = uni.upx2px(350); //350是定义消息圖片最大高度
		...
		voiceCancel() {
			...
			this.voiceTis = '押し続ける 話す';
			this.recordTis = '指を上げる 送信をキャンセルする';
			this.willStop = true; //不發送錄音
			this.RECORDER.stop(); //錄音結束
		},
		// 錄音中(判断是否触發上滑取消發送)
		voiceIng(e) {
			if (!this.recording) {
			...
			if (this.initPoint.Y - touche.clientY >= uni.upx2px(200)) {
				this.willStop = true;
				this.recordTis = '指を離します 送信をキャンセルする';
			} else {
				this.willStop = false;
				this.recordTis = '指を上げる 送信をキャンセルする';
			}
