	...
	<view class="wanl-shop">
		...
		>
			...
			<view class="shop-menu text-white" v-if="pageType">
				<view class="box">
					<view class="item" :class="{ select: filterIndex === 0 }" @tap="tabClick(0)">ホームページ</view>
					<view class="item" :class="{ select: filterIndex === 1 }" @tap="tabClick(1)">おすすめ</view>
					<view class="item" :class="{ select: filterIndex === 2 }" @tap="tabClick(2)">販売</view>
					<view class="item" :class="{ select: filterIndex === 3 }" @tap="tabClick(3)">新製品</view>
			...
			<view class="header">
				...
				<view class="text-white" style="flex: 1;">
					...
					<view class="describe text-min">
						<text>ファンの数 {{ $wanlshop.toFormat(shopData.like, 'thousand') }}</text>
					</view>
				</view>
				<!-- #ifndef MP -->
...
export default {
	data() {
		return {
			...
			pageData: {
				page: {
					params: {
						navigationBarTitleText: '店内の商品を検索する'
					},
					style: {
	...
	onPageScroll(e) {
		...
		e.scrollTop = e.scrollTop > tmpY ? tmpY : e.scrollTop; //如果当前高度大於250则250否则当前高度
