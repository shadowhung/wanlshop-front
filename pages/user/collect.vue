<template>
	<view class="wanl-collect">
				...
				<block v-if="item.goods.title">
					...
					<view class="content" @tap="onGoods(item.goods_id)">
						...
						<view class="wanl-gray text-sm">
							<text>お気に入り {{item.goods.like}}</text>
							<text class="margin-left-xs">支払い {{item.goods.payment}}</text>
						</view>
						<view class="function">
					...
					<view class="move">
						<view class="bg-orange" @tap="onShop(item.goods.shop_id)">ショップ</view>
						<view class="bg-red" @tap="loadFollow(item.goods_id, index)">削除</view>
					</view>
				</block>
				<block v-else>
					...
					<view class="content" @tap="onGoods(item.goods_id)">
						<view class="text-sm">
							<text class="text-cut-2">製品の有効期限が切れています。スライドして削除できます</text>
						</view>
						<view class="wanl-gray text-sm">
							<text>お気に入り 0</text>
							<text class="margin-left-xs">支払い 0</text>
						</view>
						<view class="function">
		...
		<wanl-empty text="コレクションが見つかりません" src="collect_default3x" v-else/>
