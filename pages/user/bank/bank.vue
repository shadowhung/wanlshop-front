	...
	<view>
		...
		<view class="margin-bj">
			<view class="item" v-for="(item, key) in bankList" :key="item.id" @tap="checkBank(key)">
				<wanl-bank :bankCode="item.bankCode" :bankName="item.bankName" :bankName2="item.bankName2" :cardCode="item.cardCode" />
		...
		<view v-if="bankList.length == 0">
			<wanl-empty text="アカウントが見つかりません"/>
		</view>
		<view class="edgeInsetBottom"></view>
		<view class="wanlian cu-bar tabbar foot flex flex-direction"><button @tap="addBank('add')" class="cu-btn wanl-bg-orange lg">口座を追加</button></view>
	</view>
</template>
