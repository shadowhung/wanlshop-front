<template>
	<view>
		<view class="auth">
			<view class="wanl-title">アカウントを入力してください</view>
			<form @submit="formSubmit">
				<view class="auth-group radius-bock bg-gray wlian-grey-light">
					<input placeholder="携帯電話を入力してください/ユーザー名" placeholder-class="placeholder" name="account" type="text" maxlength="16"
					 confirm-type="next" @input="onKeyInput"></input>
				</view>
				<view class="auth-button flex flex-direction">
					<button form-type="submit" class="cu-btn sl radius-bock bg-orange" :disabled="submitDisabled">次のステップ</button>
					<!-- #ifdef MP-WEIXIN -->
					<view class="wanl-weixin-btn-info margin-tb-bj text-center text-sm">または</view>
					<button type="primary" class="cu-btn sl radius-bock bg-no" open-type="getPhoneNumber" @getphonenumber="decryptPhoneNumber">携帯電話のワンクリックログインを取得</button>
					<!-- #endif -->
				</view>
			</form>
		</view>
	...
	export default {
		...
		onLoad(option) {
			...
			uni.getProvider({
				...
				fail: (error) => {
					console.log('ログインチャネルの取得に失敗しました', error);
				}
			});
			// #endif
			// #ifdef H5
		...
		methods: {
			...
			tologin(provider) {
				...
				uni.login({
					...
					// #ifdef MP-ALIPAY
					scopes: 'auth_user', //支付宝小程序需設置授權類型
					// #endif
					success: (loginRes) => {
						this.$api.post({
			...
			formSubmit(e) {
				...
				if (checkRes) {
					...
					if (formData.account) {
						...
						this.$wanlshop.msg('アカウント番号を入力してください');
					}
				}
			},
			register() {
