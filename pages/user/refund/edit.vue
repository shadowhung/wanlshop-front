	...
	<view>
		...
		<view class="margin-top-bj">
			...
			<view class="cu-form-group margin-top-bj">
				...
				<input placeholder="選填" v-model="refund.refund_content"></input>
			</view>
		</view>
...
export default {
	data() {
		return {
			...
			expressList: ['貨を受け取っていないで','はすでに貨を受け取りました'],
			typeList: ['私は返金して(返品する必要はない)','返品する返金'],
			reasonList: ['が好きでないで','いたずらにくるむ','ずっと配達していない','ですか？色/サイズはに一致しないで','品質の問題','少しの件が抜かして出す','はブランドを詐称します'],
			// 価格
			amount:{
				total: 0,
				freight: 0,
	...
	methods: {
			...
			this.$api.get({
				...
				success: res => {
					...
					if(res.goods_number == 1 || res.freight_type == 2){
						this.amount.info = `最大払い戻し${price + freight_price}円，送料込み${freight_price}円`;
						this.amount.total = price + freight_price;
					}else{
						this.amount.info = `最大払い戻し${price}円，配送戦略には配送は含まれません${freight_price}円`;
		...
		async addData(){
			if (this.refund.price <= 0) {
				this.$wanlshop.msg('価格を空にすることはできません');
				return false;
			}
			if (this.refund.price > this.amount.total) {
				this.$wanlshop.msg('払い戻しは超過できません'+this.amount.total+'円');
				return false;
			}
			// 提交
			this.$api.post({
