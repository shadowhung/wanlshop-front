		...
		<view class="margin-bj bg-white radius-bock" v-if="isLoad">
			...
			<view class="order-tracking">
				<view class="time-axis wanl-gray">
						...
						<block v-if="index == 0">
							...
							<view class="timeaxis-node">
								<view class="cu-tag round wanl-bg-orange">
									...
									<text class="wlIcon-wuliuqiache2" v-else-if="item.status == ''"></text>
									<text class="wlIcon-31xuanze" v-else-if="item.status == 'サインする'"></text>
									<text class="wlIcon-paisongtixing" v-else-if="item.status == 'ディスパッチ'"></text>
									<text class="wlIcon-shiyongbangzhu1" v-else-if="item.status == '難しい'"></text>
									<text class="wlIcon-daifahuo" v-else-if="item.status == '撤回'"></text>
									<text class="wlIcon-guanbi" v-else-if="item.status == '戻る'"></text>
									<text class="wlIcon-jiangjia" v-else-if="item.status == 'スイッチ'"></text>
									<text class="wlIcon-31daifukuan" v-else-if="item.status == 'まだ支払われていません'"></text>
									<text class="wlIcon-chanpincanshu" v-else-if="item.status == '注文しました'"></text>
									<text class="wlIcon-shijian" v-else-if="item.status == '倉庫処理'"></text>
								</view>
							</view>
						</block>
						<block v-else>
							...
							<view class="timeaxis-node">
								<text class="wlIcon-dot" v-if="item.status == '在途'"></text>
								<view class="cu-tag round" v-else-if="item.status == '収集する'"><text class="wlIcon-wuliuqiache2"></text></view>
								<text class="wlIcon-shiyongbangzhu1" v-else-if="item.status == '難しい'"></text>
								<text class="wlIcon-daifahuo" v-else-if="item.status == '撤回'"></text>
								<view class="cu-tag round" v-else-if="item.status == 'ディスパッチ'"><text class="wlIcon-paisongtixing"></text></view>
								<text class="wlIcon-guanbi" v-else-if="item.status == '戻る'"></text>
								<text class="wlIcon-jiangjia" v-else-if="item.status == 'スイッチ'"></text>
								<text class="wlIcon-31daifukuan" v-else-if="item.status == 'まだ支払われていません'"></text>
								<text class="wlIcon-chanpincanshu" v-else-if="item.status == '注文しました'"></text>
					...
					<view class="timeaxis-item">
						<view class="text-min">集荷待ちの小包</view>
	...
	methods: {
		...
		onCopy(text) {
			thorui.getClipboardData(text, (res) => {
				// #ifdef H5 || MP-ALIPAY
				if (res) {
					this.$wanlshop.msg('注文番号が正常にコピーされました');
				} else {
					this.$wanlshop.msg('注文番号のコピーに失敗しました');
				}
				// #endif
			})
		}
