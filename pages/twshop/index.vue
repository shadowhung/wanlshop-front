			...
			<view class="cu-bar fixed" :style="{
				...
				<view class="search-form round">
					...
					<swiper class="search-swiper placeholder" vertical autoplay circular interval="3000">
						<swiper-item @tap="productSearch('')">商品を探す、カテゴリとストア</swiper-item>
				...
				<view class="cu-dialog">
					<view class="hade">
						...
						<view class="title">
							...
							<text class="text-white text-bold5">の最新バージョン：{{update.data.versionName}}</text>
						</view>
					</view>
					
					<view class="info">
						<view class="text-lg text-bold5">
							<text>コンテンツを更新する：</text>
						</view>
						...
						<view class="margin-top-xl" v-if="update.download.start">
							...
							<view class="wanl-gray-dark text-sm text-center">
								<text>ダウンロード中、お待ちください（{{$wanlshop.conver(update.download.totalBytesWritten)}}/{{$wanlshop.conver(update.download.totalBytesExpectedToWrite)}}）</text>
							</view>
						</view>
						<!-- 開始安装 -->
						...
						<view class="flex justify-around margin-top-xl" v-else>
							<button class="cu-btn radius-bock bg-gray lg" @tap="ignore">アップグレードを無視する</button>
							<button class="cu-btn radius-bock bg-blue lg" @tap="download">今すぐアップグレード</button>
						</view>
					</view>
				</view>
			</view> 
...
export default {
		...
		uni.getNetworkType({
		    success: (res) => {
				if (res.networkType == '2g' || res.networkType == '3g' || res.networkType == '4g') {
					this.$wanlshop.msg('現在非WIFI環境を使用しているため、データ使用量に注意してください');
				}else if(res.networkType == 'none'){
					this.$wanlshop.msg('インターネットがない');
				}
		    }
		});
	},
	onPageScroll(e) {
		let tmpY = 300;
		e.scrollTop = e.scrollTop > tmpY ? 300 : e.scrollTop; //如果当前高度大於250则250否则当前高度
	...
	methods: {
		...
		userAuth() {
			this.$wanlshop.to('/pages/user/auth/name?url=/pages/twshop/index');
		},
		scanCode() {
			// #ifndef H5
			uni.scanCode({
				success: (res) => {
					...
					switch(QRcode.QRtype) {
						...
						case 'live':
							...
							this.$wanlshop.msg('現在、ライブブロードキャストで利用できるのはアプリとWeChatミニプログラムのみです')
							// #endif
							break;
						case 'chat':
			...
			// #endif
			// #ifdef H5
			this.$wanlshop.msg('H5スキャンコードを一時的にサポートしていません');
			// #endif
		},
		// 解析Url
		getUrlParam(url) {
