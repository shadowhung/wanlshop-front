<template>
	<view>
		...
		<view class="wanl-find" v-for="(find, index) in listData" :key="find.id">
			<view class="margin-bj bg-white radius-bock padding-lr-bj padding-top-bj">
					...
					<view class="avatar">
						...
						<view>
							<view>
								...
								<view v-if="find.isLive" @tap="onLive(find.isLive.id)" class="cu-tag sm bg-red round"><text class="wlIcon-zhibo"></text>生放送の中</view>
							</view>
				...
				<view class="flex justify-between padding-tb-bj align-center">
					<view class="wanl-gray-light text-sm" @tap="onFind(find)">{{find.views}} 読みます</view>
					<view class="flex">
		...
		<view v-if="listData.length == 0">
			<wanl-empty src="find_default3x" :text="'いかなる点が見つかっていないで褒めます'"/>
		</view>
